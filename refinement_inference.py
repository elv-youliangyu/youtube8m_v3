...
if __name__ == "__main__":
  ...
  flags.DEFINE_string(
      "seg_train_dir", "", "The directory to load the model files from. We assume "
      "that you have already run eval.py onto this, such that "
      "inference_model.* files already exist.")
  flags.DEFINE_string(
      "input_data_pattern", "",
...
def inference(reader, train_dir, data_pattern, out_file_location, batch_size,
  ...
  with tf.Session(config=tf.ConfigProto(
    ...
    try:
      ...
      tag_file = '.'.join(final_out_file.name.split('.')[:-1]) + "_video_tags.csv"
      if FLAGS.segment_labels:
        try:
          f = open(tag_file)
          cg_pool = {}
          for line in f:
            video_id, preds = line.strip().split(',')
            if video_id == "VideoId": continue
            preds = preds.split(" ")
            ...
            for cls, score in zip(pred_cls_ids, pred_cls_scores):
              cg_pool[video_id] = set([cls for cls, score in zip(pred_cls_ids, pred_cls_scores) if score > 0.00025])

        except IOError:
          print("candidates generation failed!!")
        ...
        for line in out_file:
          ...
          for cls, score in zip(pred_cls_ids, pred_cls_scores):
            if not whitelisted_cls_mask[cls] or cls in cg_pool[segment_id.split(':')[0]]:
              # Skip non-whitelisted classes or classes not in candidate generation pool
              continue
            if cls not in heaps:
              heaps[cls] = []
        ...
        logging.info("Writing video level predictions to: %s",
                     tag_file)
...
def main(unused_argv):
  ...
  if FLAGS.segment_labels = True:
    FLAGS.segment_labels = False
    inference(reader, FLAGS.train_dir, FLAGS.input_data_pattern,
              FLAGS.output_file, FLAGS.batch_size, 100)
    FLAGS.segment_labels = True
    inference(reader, FLAGS.seg_train_dir, FLAGS.input_data_pattern,
